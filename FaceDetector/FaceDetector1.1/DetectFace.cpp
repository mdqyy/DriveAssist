...
const char EYE_HSV_NAME[] = "/Users/xuanpengli/Developer/Capture/hsv_eye/eye_bin_%d_%d_%d.jpg";
...
namespace FaceDetector{
    ...
    void FindEyePts(const cv::Mat &src, 
                     int &top, int &bottom,
                     int &left, int &right, int &flag)
    {
        //cv::imshow("eye", src);cv::moveWindow("eye", 20, 100);
        ...
        if (bottom == 0) {
            printf("FindEyePts: Cannot Find Eyes!\n");
            return;
        }
...
//            printf("eyesRoiRect.at(0).y : %d, eyesPointVec.at(0).y : %d\n", eyesRect.at(0).y+eyesRect.at(0).height/2+faceRoiRect.y, eyesPointVec.at(0).y);
...
/**********************************/
                ...
                //pt1, pt2, center located in the original frame size
                    ...
                    //printf("DetectEyes: Cannot Find Eyes...\n");
                }
            }// for loop end
        } else {
            //printf("DetectEyes: Cannot Detect Eyes...\n");
        }
    }
