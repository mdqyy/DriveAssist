...
//            FittingCurve_LS(rightPointSet, termNum, rightCoefs);
	...
	{
        cv::Mat subMat = laneMat(cv::Rect(0, offsetY, laneMat.cols, laneMat.rows-offsetY));
        
        //! Edge Detection
        FilterLanes( subMat, laneDetectorConf );
        ...
        cv::Mat lLaneMask = cv::Mat(subMat.rows, subMat.cols, CV_8U, cv::Scalar::all(0));
        cv::Mat rLaneMask = cv::Mat(subMat.rows, subMat.cols, CV_8U, cv::Scalar::all(0));
...
//            std::cout << "Set Up ROI" << std::endl;
            ...
            const int proStartXRange = 40; //!pixel
            const int proEndXRange = 150; //!pixel
            std::vector<Lane> lastLanes;
            hfLanetoLane(subMat, pHfLanes, lastLanes);
            ...
            int lEndX = 0;
            if(pHfLanes[0][1] > 0) {
                lEndX = cvRound((lY - subMat.rows) * tan(pHfLanes[0][1]));
            } else {
                lEndX = subMat.cols - cvRound((lY - subMat.rows) * -tan(pHfLanes[0][1]));
            }
            ...
            int rEndX = subMat.cols - cvRound((rY - subMat.rows) * tan(CV_PI - pHfLanes[1][1]));
            if(pHfLanes[1][1] > 0) {
                rEndX = cvRound((rY - subMat.rows) * tan(pHfLanes[1][1]));
            } else {
                rEndX = subMat.cols - cvRound((rY - subMat.rows) * -tan(pHfLanes[1][1]));
            }
            ...
            lPoly[0][2] = cv::Point(lEndX-proEndXRange, subMat.rows);
            lPoly[0][3] = cv::Point(lEndX+proEndXRange, subMat.rows);
            ...
            rPoly[0][2] = cv::Point(rEndX-proEndXRange, subMat.rows);
            rPoly[0][3] = cv::Point(rEndX+proEndXRange, subMat.rows);
            const cv::Point *prPoly[1] = {rPoly[0]};
            cv::fillPoly(rLaneMask, prPoly, npoly, 1, cv::Scalar(255));
            
//            if(1){
//            cv::threshold(lLaneMask, lLaneMask, 0, 255, 1);
//            cv::threshold(rLaneMask, rLaneMask, 0, 255, 1);
//            }
            cv::imshow("lLaneMask", lLaneMask);
            cv::moveWindow("lLaneMask", 0, 0);
            cv::imshow("rLaneMask", rLaneMask);
        ...
        // Lane Extraction
		GetHfLanes(subMat, laneDetectorConf, hfLanes, lLaneMask, rLaneMask, isROI, isChangeLane);
    }
	...
	void FilterLanes(cv::Mat &laneMat, const LaneDetectorConf &laneDetectorConf)
	{
		...
		switch (filterType) //laneDetectorConf.filterType
        {
            case SOBEL_FILTER_1:
                ...
                if(DEBUG_LANE)imShowSub("2.Blur", mat2, WIN_COLS, WIN_ROWS, 4);
                
                cv::threshold(mat2, finalMat, 15, 255, cv::THRESH_BINARY);  
                if(DEBUG_LANE)imShowSub("3.Threshold", finalMat, WIN_COLS, WIN_ROWS, 5);
			...
			case SOBEL_FILTER_2:
                ...
                if(DEBUG_LANE)imShowSub("3.Median", mat3, WIN_COLS, WIN_ROWS, 4);
                
                cv::threshold(mat3, finalMat, 90, 255, cv::THRESH_BINARY);
                if(DEBUG_LANE)imShowSub("4.Threshold", finalMat, WIN_COLS, WIN_ROWS, 5);
...
//                if(DEBUG_LANE)imShowSub("2.Blur", mat3, WIN_COLS, WIN_ROWS, 2);
                    ...
                    const int &isROI, const int &isChangeLane)
	{
        ...
        if(!isROI)
        {
            ...
            for(int i = 0; i < (int)hfLanesCandi.size(); i++) {
...
//                    cv::threshold(laneMatRGB, laneMatRGB, 0, 255, 1);//draw
                ...
                cv::imshow("Hough Lanes Candi", laneMatRGB);
...
//            std::cout << "Detect Lanes in ROI" << std::endl;
            ...
            for(int i = 0; i < (int)leftHfLanesCandi.size(); i++) {
            ...
            if(!leftHfLanes.empty()) {
                ...
                int maxFitNum = 0;
                //! Lane Candidates
                for(int i = 0; i < (int)leftLanes.size(); i++)
                {
                    ...
                    //! Points 
                    for(int j = 0; j < (int)lLanePts.size(); j++) {
            ...
            for(int i = 0; i < (int)rightHfLanesCandi.size(); i++) {
...
//                    cv::threshold(laneMatRGB, laneMatRGB, 0, 255, 1);//1
	...
	void InitlaneDetectorConf(LaneDetectorConf &laneDetectorConf)
	{
        ...
        //! Camera Parameters
        laneDetectorConf.m = HEIGHT * COEF;    //Rows (height of Image)
        laneDetectorConf.n = WIDTH * COEF;     //Columns (width of Image)
...
//        laneDetectorConf.theta0 = CV_PI*(5.5/180);   //Camera tilted angle below the horizontal(positive)
		...
		laneDetectorConf.thetaMin = CV_PI/4;
		laneDetectorConf.thetaMax = CV_PI/9*4;
		laneDetectorConf.thetaStep = CV_PI/180;
		
		laneDetectorConf.getEndPoints = 0;
		laneDetectorConf.groupThreshold = 10;//15//20//25
		laneDetectorConf.group = 1;
...
//                }
    ...
    void GetMarkerPoints(const cv::Mat &laneMat, const std::vector<cv::Vec2f> &hfLanes, cv::Point2d &vp, cv::Point2d &corner_l, cv::Point2d &corner_r, const int &offsetY)
    {
        ...
        if(!k.empty()) {
            vp.x = (k.at(0)*lanes.at(0).startPoint.x - k.at(1)*lanes.at(1).startPoint.x - lanes.at(0).startPoint.y + lanes.at(1).startPoint.y)/(k.at(0)-k.at(1));
                    ...
                    const int &isChangeLane)
    {
        ...
        if(laneKalmanIdx > TH_KALMANFILTER && !pHfLanes.empty()) {
            //! The ROI from predicted lanes
            GetMarkerPoints(laneMat, pHfLanes, vp, corner_l, corner_r, offsetY);
            ...
            if (!isChangeLane) {
                const int proStartXRange = 40;
                const int proEndXRange = 150;
                hfLanetoLane(laneMat, pHfLanes, pLanes);
                
//                std::cout << "pHfLanes >> " << pHfLanes[0][1] << " " << pHfLanes[1][1] << std::endl;
                ...
                cv::line(laneMat, cv::Point(lStartX+proStartXRange, offsetY), cv::Point(lStartX-proStartXRange, offsetY), CV_RGB(100, 0, 100), 2);
                cv::line(laneMat, cv::Point(lStartX-proStartXRange, offsetY), cv::Point(lEndX-proEndXRange, laneMat.rows), CV_RGB(100, 0, 100), 2);
                cv::line(laneMat, cv::Point(lEndX-proEndXRange, laneMat.rows), cv::Point(lEndX+proEndXRange, laneMat.rows), CV_RGB(100, 0, 100), 2);
                cv::line(laneMat, cv::Point(lEndX+proEndXRange, laneMat.rows), cv::Point(lStartX+proStartXRange, offsetY), CV_RGB(100, 0, 100), 2);
                ...
                cv::line(laneMat, cv::Point(rStartX+proStartXRange, offsetY), cv::Point(rStartX-proStartXRange, offsetY), CV_RGB(0, 100, 100), 2);
                cv::line(laneMat, cv::Point(rStartX-proStartXRange, offsetY), cv::Point(rEndX-proEndXRange, laneMat.rows), CV_RGB(0, 100, 100), 2);
                cv::line(laneMat, cv::Point(rEndX-proEndXRange, laneMat.rows), cv::Point(rEndX+proEndXRange, laneMat.rows), CV_RGB(0, 100, 100), 2);
                cv::line(laneMat, cv::Point(rEndX+proEndXRange, laneMat.rows), cv::Point(rStartX+proStartXRange, offsetY), CV_RGB(0, 100, 100), 2);
...
//            cv::fillPoly(laneMat, ppoly, npoly, 1, cv::Scalar(255, 0, 0));
                    ...
                    const double &lateralOffset)
    {
        if ((int)hfLanes.size() == 2){
            cv::Point2d  vp, corner_l, corner_r;
            const int offsetY = cvRound(laneMat.rows * YAW_ANGLE);
            GetMarkerPoints(laneMat, hfLanes, vp, corner_l, corner_r, offsetY);
